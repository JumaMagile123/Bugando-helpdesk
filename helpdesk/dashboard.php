    ...
    <div class="row">
        ...
        <main class="col p-0">
            ...
            <div class="p-4">
                ...
                <div class="card panel-card mt-4 p-4" id="incoming-requests"><div class="d-flex justify-content-between align-items-center mb-3"><h5 class="mb-0">Request queue</h5><form method="GET"><select name="status" class="form-select form-select-sm" onchange="this.form.submit()"><option value="open" <?= $filter_status === 'open' ? 'selected' : '' ?>>Open requests</option><?php foreach (['pending', 'assigned', 'in_progress', 'escalated', 'resolved', 'closed'] as $status): ?><option value="<?= $status ?>" <?= $filter_status === $status ? 'selected' : '' ?>><?= ucwords(str_replace('_', ' ', $status)) ?></option><?php endforeach; ?></select></form></div>
